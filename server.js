...
wss.on('connection', (ws) => {
  ...
  try {
	...
	dgWs = new WebSocket(`wss://api.deepgram.com/v1/listen?model=${languageModel}&language=${language}&encoding=linear16&sample_rate=16000&channels=1&interim_results=true&endpointing=300`, {
	  headers: { 'Authorization': `Token ${DEEPGRAM_API_KEY}` }
	});
    

    // Initialisation Deepgram - ne pas envoyer de configuration JSON
	dgWs.onopen = () => {
	console.log('Connexion Deepgram établie');
  
  // Aucune configuration JSON à envoyer ici - tout est dans l'URL
  // Vous pouvez éventuellement envoyer un message KeepAlive
	  setInterval(() => {
